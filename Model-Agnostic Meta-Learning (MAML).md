...
如同在section 2.1.中的描述（在獨立且同分布的監督式學習問題，$H=1$），這裡考慮$H=1$<font color="green">（具dependent關係的樣本長度為1）</font>，並將timestep $t$標記於$\textbf{x}$上，如$\textbf{x}_t$，因為模型接受單個輸入並產生單個輸出，而不是輸入和輸出序列。
